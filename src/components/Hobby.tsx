...
const Hobby = async () => {
  ...
  return (
    <section id="hobby" className="content-auto py-28 px-5 sm:px-8 max-w-6xl mx-auto">
      ...
      <div className="grid md:grid-cols-2 gap-5">
        {hobbiesData.map((hobby, i) => {
          const highlights = t.raw(`items.${hobby.id}.highlights`) as string[];
          return (
            <MotionDiv
              key={hobby.id}
              ...
              <div className="mirror-card rounded-sm h-full flex flex-col">
                ...
                <div className="flex items-center justify-between px-5 py-3 border-b border-silver-100">
                  <div className="flex items-center gap-2">
                    ...
                    <Text variant="overline" className="tracking-nav uppercase text-silver-bright drop-shadow-[0_0_8px_rgba(192,192,200,0.85)]">
                      hobby_{String(i).padStart(2, "0")}
                    </Text>
                  </div>
                  <div className="flex items-center gap-3">
                    {hobby.noteLink && (
                      <a
                        ...
                        className="text-sm lg:text-md tracking-btn text-silver-bright drop-shadow-[0_0_8px_rgba(192,192,200,0.85)] uppercase p-2 -m-2"
                        aria-label={`Read note article about ${t(`items.${hobby.id}.title`)} (opens in new tab)`}
                      >
                        note →
                      </a>
                    )}
                    <a
                      href={hobby.githubLink}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-sm lg:text-md tracking-btn text-silver-bright drop-shadow-[0_0_8px_rgba(192,192,200,0.85)] uppercase p-2 -m-2"
                      aria-label={`View ${t(`items.${hobby.id}.title`)} on GitHub (opens in new tab)`}
                    >
                      github →
                    </a>
                  </div>
